import json
import os
import sys

project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
if project_root not in sys.path:
    sys.path.insert(0, project_root)
from markdown_analyzer_lib.markdown_analyzer import MarkdownAnalyzer

# Path to the test markdown file
data_file_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'test_code.md')
# Fallback
# data_file_path = "data/test_code.md"

try:
    print(f"Attempting to load Markdown file from: {data_file_path}")
    if not os.path.exists(data_file_path):
        alt_data_file_path = "data/test_code.md"
        if os.path.exists(alt_data_file_path):
            data_file_path = alt_data_file_path
            print(f"Using fallback data file path: {data_file_path}")
        else:
            raise FileNotFoundError(f"Markdown file not found at {data_file_path} or {alt_data_file_path}")

    analyzer = MarkdownAnalyzer(file_path=data_file_path)
    print("MarkdownAnalyzer initialized successfully.")

    # Use case for identify_task_items() (aliased as identify_todos)
    # The method `identify_task_items` is used for this.
    print("\n--- Identified Task Items (Todos) ---")
    task_items_data = analyzer.identify_task_items() # Returns a list of dicts

    if task_items_data:
        print(f"Found {len(task_items_data)} task item(s):")
        for i, task_info in enumerate(task_items_data):
            status = "[x]" if task_info.get('checked') else "[ ]"
            print(f"\nTask Item {i+1}:")
            print(f"  Line: {task_info.get('line')}")
            print(f"  Status: {status}")
            print(f"  Text: {task_info.get('text')}")
        
        # For a more structured JSON output if needed:
        # print("\nFull JSON output for task items:")
        # print(json.dumps(task_items_data, indent=2))
    else:
        print("No task items (todos) found or an issue with identification.")
        print("Ensure 'test_code.md' contains task list items, e.g.:")
        print("  - [ ] Unchecked task")
        print("  - [x] Checked task")
        print("  * [ ] Another unchecked task")

except FileNotFoundError as fnf_error:
    print(f"Error: {fnf_error}")
    print(f"Current working directory: {os.getcwd()}")
    print("Please ensure the 'data/test_code.md' file exists in the correct location.")
except Exception as e:
    print(f"An unexpected error occurred: {e}")
