...
try:
    ...
    # Use case for identify_code_blocks()
    print("\n--- Identified Code Blocks ---")
    code_blocks_data = analyzer.identify_code_blocks()

    if code_blocks_data.get("Code block"):
        print(f"Found {len(code_blocks_data['Code block'])} code block(s):")
        for i, cb_info in enumerate(code_blocks_data["Code block"]):
            print(f"\nCode Block {i+1}:")
            print(f"  Line: {cb_info.get('start_line')}")
            print(f"  Language: {cb_info.get('language', 'Not specified')}")
            print(f"  Type: {cb_info.get('code_type', 'Not specified')}")
            print(f"  Content:\n{cb_info.get('content')}")
        # For a more structured output if needed:
        # print(json.dumps(code_blocks_data, indent=2))
    else:
        print("No code blocks found or an issue with code block identification.")
        print("Ensure 'test_code.md' contains fenced (```) or indented code blocks.")
